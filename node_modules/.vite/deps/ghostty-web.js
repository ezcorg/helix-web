//#region node_modules/.pnpm/ghostty-web@0.4.0/node_modules/ghostty-web/dist/ghostty-web.js
...
var q = class q {
	...
	static async load(A) {
		...
		throw E || /* @__PURE__ */ new Error("Failed to load Ghostty WASM");
	}
	static async loadFromPath(A) {
		let B;
		if (typeof Bun < "u" && typeof Bun.file == "function") try {
			const C = Bun.file(A);
			await C.exists() && (B = await C.arrayBuffer());
		} catch {}
		if (!B) try {
			const I = await (await import("./__vite-browser-external-2447137e-C0JxZmhB.js")).readFile(A);
			B = I.buffer.slice(I.byteOffset, I.byteOffset + I.byteLength);
		} catch {}
		if (!B) {
			const C = await fetch(A);
			if (!C.ok) throw new Error(`Failed to fetch WASM: ${C.status} ${C.statusText}`);
			if (B = await C.arrayBuffer(), B.byteLength === 0) throw new Error(`WASM file is empty (0 bytes). Check path: ${A}`);
		}
		if (!B) throw new Error(`Could not load WASM from path: ${A}`);
		const g = await WebAssembly.compile(B), E = await WebAssembly.instantiate(g, { env: { log: (C, I) => {
...
var V = class {
	constructor(A) {
		...
		this.encoder = new DataView(this.exports.memory.buffer).getUint32(B, !0), this.exports.ghostty_wasm_free_opaque(B);
	}
	setOption(A, B) {
	...
	encode(A) {
		...
		const I = 32, D = this.exports.ghostty_wasm_alloc_u8_array(I), i = this.exports.ghostty_wasm_alloc_usize(), w = this.exports.ghostty_key_encoder_encode(this.encoder, C, D, I, i);
		if (w !== 0) throw this.exports.ghostty_wasm_free_u8_array(D, I), this.exports.ghostty_wasm_free_usize(i), this.exports.ghostty_key_event_free(C), /* @__PURE__ */ new Error(`Failed to encode key: ${w}`);
...
var z = class K {
	constructor(A, B, g = 80, E = 24, C) {
		...
		if (this.viewportBufferPtr = 0, this.viewportBufferSize = 0, this.cellPool = [], this.graphemeBuffer = null, this.graphemeBufferPtr = 0, this.exports = A, this.memory = B, this._cols = g, this._rows = E, C) {
			...
			try {
				...
				for (let s = 0; s < 16; s++) i.setUint32(w, ((I = C.palette) == null ? void 0 : I[s]) ?? 0, !0), w += 4;
	...
	*/
	getScrollbackLine(A) {
		...
		const g = this.exports.ghostty_terminal_get_scrollback_line(this.handle, A, this.viewportBufferPtr, this._cols);
		if (g < 0) return null;
	...
	*/
	...
	initCellPool() {
		...
		if (this.cellPool.length < A) for (let B = this.cellPool.length; B < A; B++) this.cellPool.push({
			codepoint: 0,
			fg_r: 204,
			fg_g: 204,
			fg_b: 204,
	...
	*/
	getGrapheme(A, B) {
		...
		const g = this.exports.ghostty_render_state_get_grapheme(this.handle, A, B, this.graphemeBufferPtr, 16);
		if (g < 0) return null;
	...
	*/
	getScrollbackGrapheme(A, B) {
		this.graphemeBuffer || (this.graphemeBufferPtr = this.exports.ghostty_wasm_alloc_u8_array(64), this.graphemeBuffer = new Uint32Array(this.memory.buffer, this.graphemeBufferPtr, 16));
		const g = this.exports.ghostty_terminal_get_scrollback_grapheme(this.handle, A, B, this.graphemeBufferPtr, 16);
		if (g < 0) return null;
		const E = new Uint32Array(this.memory.buffer, this.graphemeBufferPtr, g);
		return Array.from(E);
	}
...
var J = class {
	constructor() {
		this.listeners = [], this.event = (A) => (this.listeners.push(A), { dispose: () => {
...
var m = class {
	...
	getLine(A) {
		...
		} else C = this.bufferType === "normal" ? A - g : A, E = B.getLine(C), I = B.isRowWrapped(C);
		if (E) return new j(E, I, B.cols);
	}
	getNullCell() {
...
var j = class {
	...
	getCell(A) {
		if (!(A < 0 || A >= this._length)) return A >= this.cells.length ? new l({
			codepoint: 0,
			fg_r: 204,
			fg_g: 204,
			fg_b: 204,
...
var P = class {
	...
	*/
	handleKeyDown(A) {
		if (this.isDisposed || this.isComposing || A.isComposing || A.keyCode === 229) return;
		if (this.onKeyCallback && this.onKeyCallback({
			key: A.key,
			domEvent: A
		}), this.customKeyEventHandler && this.customKeyEventHandler(A)) {
			A.preventDefault();
			return;
		}
	...
	*/
	handleCompositionEnd(A) {
		...
		this.isComposing = !1;
		const B = A.data;
		if (B && B.length > 0 && this.onDataCallback(B), this.container && this.container.childNodes) for (let g = this.container.childNodes.length - 1; g >= 0; g--) {
...
var v = class {
	...
	*/
	async getLinkAt(A, B) {
		...
		for (const I of this.linkCache.values()) if (this.isPositionInLink(A, B, I)) return I;
	}
	/**
...
var $ = class {
	constructor(A, B = {}) {
		this.cursorVisible = !0, this.lastCursorPosition = {
			x: 0,
			y: 0
		}, this.lastViewportY = 0, this.currentBuffer = null, this.currentSelectionCoords = null, this.hoveredHyperlinkId = 0, this.previousHoveredHyperlinkId = 0, this.hoveredLinkRange = null, this.previousHoveredLinkRange = null, this.canvas = A;
		const g = A.getContext("2d", { alpha: !0 });
		if (!g) throw new Error("Failed to get 2D rendering context");
		this.ctx = g, this.fontSize = B.fontSize ?? 15, this.fontFamily = B.fontFamily ?? "monospace", this.cursorStyle = B.cursorStyle ?? "block", this.cursorBlink = B.cursorBlink ?? !1, this.theme = {
			...f,
			...B.theme
		}, this.devicePixelRatio = B.devicePixelRatio ?? window.devicePixelRatio ?? 1, this.palette = [
			this.theme.black,
			this.theme.red,
			this.theme.green,
			this.theme.yellow,
	...
	measureFont() {
		...
		const g = B.measureText("M"), E = Math.ceil(g.width), C = g.actualBoundingBoxAscent || this.fontSize * .8, I = g.actualBoundingBoxDescent || this.fontSize * .2;
		return {
			width: E,
			height: Math.ceil(C + I) + 2,
			baseline: Math.ceil(C) + 1
		};
	}
	/**
	...
	*/
	render(A, B = !1, g = 0, E, C = 1) {
		...
		if (this.currentSelectionCoords = N ? this.selectionManager.getSelectionCoords() : null, this.currentSelectionCoords) {
			...
			for (let c = t.startRow; c <= t.endRow; c++) k.add(c);
		}
		if (this.selectionManager) {
			const t = this.selectionManager.getDirtySelectionRows();
			if (t.size > 0) {
				for (const c of t) k.add(c);
		...
		if (a) {
			for (let t = 0; t < D.rows; t++) {
				...
				if (c) {
					for (const F of c) if (F.hyperlink_id === this.hoveredHyperlinkId || F.hyperlink_id === this.previousHoveredHyperlinkId) {
						M.add(t);
						break;
					}
				}
		...
		if (h) {
			if (this.previousHoveredLinkRange) for (let t = this.previousHoveredLinkRange.startY; t <= this.previousHoveredLinkRange.endY; t++) M.add(t);
			if (this.hoveredLinkRange) for (let t = this.hoveredLinkRange.startY; t <= this.hoveredLinkRange.endY; t++) M.add(t);
			this.previousHoveredLinkRange = this.hoveredLinkRange;
		}
		const G = /* @__PURE__ */ new Set();
		for (let t = 0; t < D.rows; t++) (g > 0 || B || A.isRowDirty(t) || k.has(t) || M.has(t)) && (G.add(t), t > 0 && G.add(t - 1), t < D.rows - 1 && G.add(t + 1));
		for (let t = 0; t < D.rows; t++) {
			if (!G.has(t)) continue;
			let c = null;
			if (g > 0) if (t < g && E) {
		...
		g === 0 && I.visible && this.cursorVisible && this.renderCursor(I.x, I.y), E && C > 0 && this.renderScrollbar(g, i, D.rows, C), this.lastCursorPosition = {
			x: I.x,
			y: I.y
		}, A.clearDirty();
	}
	...
	*/
	renderCellText(A, B, g) {
		var k;
		const E = B * this.metrics.width, C = g * this.metrics.height, I = this.metrics.width * A.width;
		if (A.flags & e.INVISIBLE) return;
		const D = this.isInSelection(B, g);
		let i = "";
		if (A.flags & e.ITALIC && (i += "italic "), A.flags & e.BOLD && (i += "bold "), this.ctx.font = `${i}${this.fontSize}px ${this.fontFamily}`, D) this.ctx.fillStyle = this.theme.selectionForeground;
		else {
			let M = A.fg_r, a = A.fg_g, h = A.fg_b;
			A.flags & e.INVERSE && (M = A.bg_r, a = A.bg_g, h = A.bg_b), this.ctx.fillStyle = this.rgbToCSS(M, a, h);
		}
		A.flags & e.FAINT && (this.ctx.globalAlpha = .5);
	...
	renderCursor(A, B) {
		...
		switch (this.ctx.fillStyle = this.theme.cursor, this.cursorStyle) {
			case "block":
				...
				break;
			case "underline":
				const C = Math.max(2, Math.floor(this.metrics.height * .15));
				this.ctx.fillRect(g, E + this.metrics.height - C, this.metrics.width, C);
				break;
			case "bar":
				const I = Math.max(2, Math.floor(this.metrics.width * .15));
	...
	*/
	renderScrollbar(A, B, g, E = 1) {
		...
		if (C.fillStyle = this.theme.background, C.fillRect(w - 2, 0, i + 6, I), E <= 0 || B === 0) return;
		const k = B + g, M = Math.max(20, g / k * N), a = A / B, h = s + (N - M) * (1 - a);
		C.fillStyle = `rgba(128, 128, 128, ${.1 * E})`, C.fillRect(w, s, i, N);
		C.fillStyle = `rgba(128, 128, 128, ${(A > 0 ? .5 : .3) * E})`, C.fillRect(w, h, i, M);
	}
	getMetrics() {
		return { ...this.metrics };
	}
	...
	getSelection() {
		...
		for (let D = B; D <= E; D++) {
			...
			for (let M = s; M <= N; M++) {
				...
				if (a && a.codepoint !== 0) {
					let h;
					if (a.grapheme_len > 0) if (D < C) h = this.wasmTerm.getScrollbackGraphemeString(D, M);
					else {
						const G = D - C;
						h = this.wasmTerm.getGraphemeString(G, M);
					}
					else h = String.fromCodePoint(a.codepoint);
...
`);
	...
	*/
	clearSelection() {
		...
		if (A) for (let B = A.startRow; B <= A.endRow; B++) this.dirtySelectionRows.add(B);
		...
		A.addEventListener("mousedown", (B) => {
			...
			if (this.isSelecting) {
				...
				this.selectionEnd = {
					col: g.col,
					absoluteRow: E
				}, this.requestRender(), this.updateAutoScroll(B.offsetY, A.clientHeight);
			}
			...
			if (this.isSelecting || this.mouseDownTarget && A.contains(this.mouseDownTarget)) return;
			const E = B.target;
		...
		this.autoScrollInterval !== null && this.autoScrollDirection === A || (this.stopAutoScroll(), this.autoScrollDirection = A, this.autoScrollInterval = setInterval(() => {
			...
			if (this.terminal.scrollLines(B), this.selectionEnd) {
				...
				if (this.autoScrollDirection < 0) {
					...
					E < this.selectionEnd.absoluteRow && (this.selectionEnd = {
						col: 0,
						absoluteRow: E
					});
				} else {
	...
	*/
	normalizeSelection() {
		...
		return I < 0 || C > i ? null : (C < 0 && (C = 0, A = 0), I > i && (I = i, g = D.cols - 1), {
			startCol: A,
			startRow: C,
			endCol: g,
			endRow: I
...
var IA = class {
	constructor(A = {}) {
		this.unicode = { get activeVersion() {
			...
			var C, I, D;
			if (g.preventDefault(), g.stopPropagation(), this.customWheelEventHandler && this.customWheelEventHandler(g)) return;
			if (((C = this.wasmTerm) == null ? void 0 : C.isAlternateScreen()) ?? !1) {
				...
				for (let s = 0; s < w; s++) i === "up" ? this.dataEmitter.fire("\x1B[A") : this.dataEmitter.fire("\x1B[B");
			} else {
				let i;
				if (g.deltaMode === WheelEvent.DOM_DELTA_PIXEL) {
					const w = ((D = (I = this.renderer) == null ? void 0 : I.getMetrics()) == null ? void 0 : D.height) ?? 20;
					i = g.deltaY / w;
				} else g.deltaMode === WheelEvent.DOM_DELTA_LINE ? i = g.deltaY : g.deltaMode === WheelEvent.DOM_DELTA_PAGE ? i = g.deltaY * this.rows : i = g.deltaY / 33;
				if (i !== 0) {
			...
			if (I >= N && I <= N + s) {
				...
				if (D >= t && D <= t + G) this.isDraggingScrollbar = !0, this.scrollbarDragStart = D, this.scrollbarDragStartViewportY = this.viewportY, this.canvas && (this.canvas.style.userSelect = "none", this.canvas.style.webkitUserSelect = "none");
				else {
		...
		this.options = new Proxy(B, { set: (g, E, C) => {
			const I = g[E];
	...
	*/
	open(A) {
		...
		try {
			...
			this.canvas.addEventListener("mousedown", (E) => {
			...
			}), this.renderer.resize(this.cols, this.rows), this.inputHandler = new P(this.ghostty, A, (E) => {
			...
			}), this.selectionManager = new AA(this, this.renderer, this.wasmTerm, this.textarea), this.renderer.setSelectionManager(this.selectionManager), this.selectionManager.onSelectionChange(() => {
				...
				passive: !1,
				capture: !0
			}), this.renderer.render(this.wasmTerm, !0, this.viewportY, this, this.scrollbarOpacity), this.startRenderLoop(), this.focus();
		} catch (B) {
			throw this.isOpen = !1, this.cleanupComponents(), /* @__PURE__ */ new Error(`Failed to open terminal: ${B}`);
		}
	}
	/**
...
`)), this.writeInternal(A, B);
	...
	*/
	writeln(A, B) {
		if (typeof A == "string") this.write(A + `\r
`, B);
		else {
	...
	*/
	resize(A, B) {
		...
		this.canvas.width = g.width * A, this.canvas.height = g.height * B, this.canvas.style.width = `${g.width * A}px`, this.canvas.style.height = `${g.height * B}px`, this.resizeEmitter.fire({
			cols: A,
			rows: B
	...
	*/
	registerLinkProvider(A) {
		if (!this.linkDetector) throw new Error("Terminal must be opened before registering link providers");
	...
	*/
	assertOpen() {
		if (this.isDisposed) throw new Error("Terminal has been disposed");
		if (!this.isOpen) throw new Error("Terminal must be opened before use. Call terminal.open(parent) first.");
	}
	/**
	...
	*/
	processMouseMove(A) {
		...
		} else D = this.wasmTerm.getLine(C);
		...
		if (a > 0) if (C < a) k = N - a + C;
		else k = N + (C - a);
		else k = N + C;
		this.linkDetector.getLinkAt(g, k).then((h) => {
			var G, U, t, c;
			if (h !== this.currentHoveredLink && ((U = (G = this.currentHoveredLink) == null ? void 0 : G.hover) == null || U.call(G, !1), this.currentHoveredLink = h, (t = h == null ? void 0 : h.hover) == null || t.call(h, !0), this.element && (this.element.style.cursor = h ? "pointer" : "text"), this.renderer)) if (h) {
	...
	*/
	processScrollbarDrag(A) {
		if (!this.canvas || !this.renderer || !this.wasmTerm || this.scrollbarDragStart === null) return;
		const B = this.wasmTerm.getScrollbackLength();
		if (B === 0) return;
		const g = this.canvas.getBoundingClientRect(), C = A.clientY - g.top - this.scrollbarDragStart, i = g.height - 8, w = this.rows, s = B + w, N = Math.max(20, w / s * i), k = -C / (i - N), M = Math.round(k * B), a = this.scrollbarDragStartViewportY + M;
...
var DA = class {
	...
	*/
	proposeDimensions() {
		var G;
		if (!((G = this._terminal) != null && G.element)) return;
		...
		if (s === 0 || N === 0) return;
		const k = s - i - w - gA, M = N - I - D;
		return {
			cols: Math.max(QA, Math.floor(k / g.width)),
			rows: Math.max(BA, Math.floor(M / g.height))
		};
	}
	/**
...
function CA() {
	if (!R) throw new Error(`ghostty-web not initialized. Call init() before creating Terminal instances.
Example:
...
//#endregion
export { $ as CanvasRenderer, e as CellFlags, J as EventEmitter, DA as FitAddon, q as Ghostty, W as GhosttyTerminal, P as InputHandler, V as KeyEncoder, H as KeyEncoderOption, v as LinkDetector, X as OSC8LinkProvider, AA as SelectionManager, IA as Terminal, _ as UrlRegexProvider, CA as getGhostty, oA as init };
